...
const DASHBOARD_NAV = [
  {
    label: "Talent & Placements",
    href: "/dashboard/institute/students",
    accentColor: "from-orange-600 to-red-600",
    spotlight: {
      badge: "Recruitment",
      title: "Placement Drives",
      description: "Track student readiness and coordinate campus recruitment drives.",
      metricValue: "112",
      metricLabel: "Active Drives",
      ctaText: "View Drives",
      ctaHref: "/dashboard/institute/placements",
    },
    columns: [
      {
        title: "Students",
        items: [
          { title: "Student Directory", href: "/dashboard/institute/students", icon: Users },
          { title: "Skill Heatmaps", href: "/dashboard/institute/heatmaps", icon: Activity },
        ]
      },
      {
        title: "Industry",
        items: [
          { title: "Placement Drives", href: "/dashboard/institute/placements", icon: Briefcase },
        ]
      }
    ]
  },
  {
    ...
    href: "/dashboard/institute/departments",
    accentColor: "from-red-600 to-rose-600",
    spotlight: {
      badge: "Insights",
      title: "Department Analytics",
      description: "Monitor department performance, placement stats, and generate reports.",
      metricValue: "100%",
      metricLabel: "NAAC Ready",
      ctaText: "View Reports",
      ctaHref: "/dashboard/institute/reports",
    },
    columns: [
      {
        title: "Analytics",
        items: [
          { title: "Department Overview", href: "/dashboard/institute/departments", icon: Building2 },
          { title: "Compliance Reports", href: "/dashboard/institute/reports", icon: FileBarChart },
        ]
      }
    ]
  }
...
export default function InstituteDashboardLayout({
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground font-sans relative z-10 selection:bg-foreground selection:text-background role-institute">
      ...
      <header className="sticky top-0 z-50 w-full glass-navbar border-b border-border/40 transition-colors bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-6">
            <Link href="/dashboard/institute" className="flex items-center gap-3 group">
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-border bg-card shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 p-1 flex flex-col">
                <div className="px-3 py-2 border-b border-border/40 mb-1">
                  <p className="text-sm font-semibold text-foreground">IIT Bombay</p>
                  <p className="text-xs text-muted-foreground">Placement Cell</p>
                </div>
                
                <DropdownItem href="/dashboard/institute/settings" icon={Settings} label="Settings" />
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 w-full bg-background pb-20 md:pb-0 relative flex flex-col">
        <div className="absolute inset-0 pointer-events-none opacity-40 dark:opacity-10 bg-[radial-gradient(at_85%_15%,_rgba(200,73,38,0.15)_0px,_transparent_50%),radial-gradient(at_15%_85%,_rgba(200,73,38,0.1)_0px,_transparent_50%)]" />
        
        <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 w-full flex-1">
          {children}
        </div>
        
        <DashboardFooter portalName="Institution" />
      </main>
      ...
      <nav className="fixed bottom-0 w-full z-50 flex justify-around items-center px-2 py-2 bg-background/90 backdrop-blur-md border-t border-border/40 md:hidden pb-safe">
        <MobileNavItem href="/dashboard/institute" icon={Home} label="Home" current={pathname === "/dashboard/institute"} />
        <MobileNavItem href="/dashboard/institute/students" icon={Users} label="Students" current={pathname.includes("/students")} />
        <MobileNavItem href="/dashboard/institute/placements" icon={Briefcase} label="Placements" current={pathname.includes("/placements")} />
        <MobileNavItem href="/dashboard/institute/departments" icon={Building2} label="Analytics" current={pathname.includes("/departments")} />
        <MobileNavItem href="/dashboard/institute/settings" icon={Settings} label="Settings" current={pathname.includes("/settings")} />
      </nav>
    </div>
  );
}
